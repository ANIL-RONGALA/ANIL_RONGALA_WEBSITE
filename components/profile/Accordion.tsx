      ...
      {items.map((item) => {
        ...
        return (
          <div key={item.id} className="rounded-2xl border border-border/70 bg-background/40">
            <button
              type="button"
              className="focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/60 flex w-full items-center justify-between gap-3 px-4 py-3 text-left transition-colors duration-200"
            ...
            >
              <div className="overflow-hidden">
                <div className="px-4 pb-4 text-sm leading-relaxed text-muted-foreground">{item.body}</div>
              </div>
            </div>
          </div>
        );
