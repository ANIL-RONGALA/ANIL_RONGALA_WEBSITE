...
export function AdCarousel() {
  return (
    <div className="overflow-hidden rounded-2xl border border-border bg-card/70 backdrop-blur transition-colors duration-500">
      <motion.div
          ...
          >
            ...
            <div className="mt-3 flex flex-wrap gap-2 text-[10px] text-muted-foreground">
              {item.tags.map((tag) => (
                <span
                  key={tag}
                  className="rounded-full border border-border/60 bg-muted/60 px-2 py-1 text-muted-foreground"
                >
                  {tag}
                </span>
              ))}
